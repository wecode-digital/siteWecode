...
function MenuMob() {
  ...
  useEffect(() => {
    ...
    sections.forEach(sectionClass => {
      const button = document.querySelector(`.${sectionClass}`);
      if (button) {
        button.addEventListener("click", () => handleScrollToSection(`.${sectionClass}`));
      }
    });
    ...
    return () => {
      sections.forEach(sectionClass => {
        const button = document.querySelector(`.${sectionClass}`);
        if (button) {
          button.removeEventListener("click", () => handleScrollToSection(`.${sectionClass}`));
        }
      });
    };
  }, []);
